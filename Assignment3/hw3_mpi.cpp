...
void draw_graph(Display *display, Window win, GC gc, body_t *body, int body_num)
{
...
void NbodyMpi(int argc, char **argv)
{
    ...
    for (k = 0; k < Iteration; k++)
    {
        ...
        MPI_Gatherv(local_body, send_count[my_rank], MPI_body_t, body, send_count,
            displacement, MPI_body_t, MASTER, MPI_COMM_WORLD);

        if (my_rank == MASTER)
        {
            if (PIC)
                draw_graph(display, win, gc, body, body_num); 
            for (i = 1; i < comm_sz; i++)
                MPI_Send(body, body_num, MPI_body_t, i, i, MPI_COMM_WORLD);
        }
    }
